...
class GameEngine:
    ...
    def _create_pieces(self) -> Tuple[List[Piece], List[Piece]]:
        ...
        def add_pieces_for_color(color: str, cols: List[int], target_list: List[Piece]):
            """Place uniquement les pièces demandées en respectant une priorité stratégique.

            - cols[0] : colonne arrière (roi, reine, autres pièces importantes)
            - cols[1] : colonne avant (pièces défensives, tours, pions)

            Règle stratégique :
            - Les tours (défensives) sont placées en avant, côté centre :
              * Blancs : colonne 1, des lignes du bas vers le haut
              * Noirs : colonne BOARD_COLS-2, des lignes du haut vers le bas
            - Roi et Reine sont placés en arrière dans la colonne 0/7, derrière les tours :
              roi d'abord (plus proche des tours), puis reine.
            - Les autres pièces (fous, cavaliers, pions) remplissent les cases restantes.
            """
            ...
            for kind in ["king", "queen"]:
                for row in back_rows:
                    if remaining.get(kind, 0) <= 0:
                        break
                    # Vérifier si une pièce est déjà sur cette case pour éviter les doublons
            ...
            other_order_front = ["bishop", "knight", "pawn"]
            other_order_back = ["bishop", "knight", "pawn"]

            # D'abord sur la colonne avant
            for kind in other_order_front:
                for row in front_rows:
                    if remaining.get(kind, 0) <= 0:
                        break
            ...
            # Puis sur la colonne arrière si encore de la place
            for kind in other_order_back:
                for row in back_rows:
                    if remaining.get(kind, 0) <= 0:
                        break
                    occupied = any((p.row == row and p.col == back_col) for p in target_list)
